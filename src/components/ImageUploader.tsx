...
interface Props {
  userId: string
  onInsert: (url: string) => void  // called with the public URL to insert into content
}
...
export default function ImageUploader({ userId, onInsert }: Props) {
  ...
  async function handleFile(file: File) {
    if (!file.type.startsWith('image/')) { setError('მხოლოდ სურათები'); return }
    if (file.size > 5 * 1024 * 1024) { setError('მაქსიმუმ 5MB'); return }
