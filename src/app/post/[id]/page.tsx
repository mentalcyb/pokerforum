  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-4">
        <div className="text-xs text-brand-600 font-medium mb-2">{post.categories?.name}</div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-3">{post.title}</h1>
        <div className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          {t.postedBy} <span className="font-medium text-gray-700 dark:text-gray-300">{post.profiles?.username}</span> · {timeAgo(post.created_at)}
        </div>
      ...
      {replies.map(r => (
        <div key={r.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 mb-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-gray-900 dark:text-white">{r.profiles?.username}</span>
